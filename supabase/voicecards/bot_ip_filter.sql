...
--   ④ select public.vc_reconcile_real_users()  ← 소급 변경이라 증분(창 안만 본다)으로는 못 지운다
--      (2026-09-07 이전에는 'refresh materialized view mv_real_users' 였다. 지금 mv_real_users 는
--       증분 동기화되는 실테이블이다 — incremental_real_users.sql 참고.)
--
...
--   108.177.0.0/17    Google Play 심사/프리런치 (2026-09-07 추가)
...
-- 108.177.0.0/17 근거: 1.1.151 안드로이드 트래픽을 CEO가 잡아냈다(2026-09-07). 이 대역에서
-- 20개 기기가 관측됐는데 **전부** 같은 모양이다: android, 릴리스마다 정확히 한 번, 0~426초
-- 버스트 후 재방문 없음, 버전이 릴리스를 그대로 따라간다(1.0.81·1.1.28·32·43·49·54·55·61·71·
-- 90·99·119·132·136·146·149·151). 실사용자는 한 명도 섞여 있지 않다.
-- 결정적 증거: 이 대역에서 로그인한 3계정이 전부 @cloudtestlabaccounts.com(구글 Firebase
-- Test Lab / 프리런치 리포트)이다 — 그 도메인은 이미 제외 목록에 있었으므로 유저 테이블
-- 쪽은 막혀 있었고, 이벤트 통계 쪽 CIDR 만 빠져 있었다.
-- 관측 IP 는 108.177.6.x / 24.x / 29.x 로 108.177.0.0/19 안에 모이지만, 74.125.0.0/16 ·
-- 192.178.0.0/16 과 같은 구글 소유 블록 전체를 넣는 기존 방식에 맞춰 /17 로 잡았다.
-- 애플과 달리 심사자가 로그인까지 하므로 유저 테이블·이벤트 통계 양쪽을 봐야 한다.
--
-- 이 건은 왜 iOS 상한 필터에 안 걸렸나: ios_ceiling/isReviewBuild 는 iOS 전용이다.
-- 같은 날 iOS 1.1.151 심사 기기는 애플 대역이라 트리거가 이미 봇으로 찍었고(50건 전부),
-- 안드로이드만 새 대역이라 새어 들어왔다. 두 규칙은 보완 관계다.
...
-- ④ select public.vc_reconcile_real_users();  -- 전체 대조(소급 변경 반영)
