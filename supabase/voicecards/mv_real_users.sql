...
-- 해결: 무거운 dedup을 매시 1회만(pg_cron REFRESH) 계산하고, 소비자는 인덱스된 스냅샷을 읽음.
...
-- 1시간 주기 새로고침 (CONCURRENTLY = 새로고침 중에도 읽기 blocking 없음)
-- cron 잡 3(매시 정각)이 refresh_vc_mvs() 로 이 MV + mv_user_latest_meta 를 순차 CONCURRENTLY 리프레시.
