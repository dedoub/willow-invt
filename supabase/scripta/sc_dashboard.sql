...
-- 유저별 첫 연습 시도(연습 시작)
first_attempt as (
  select user_id, min(created_at) as first_at from scripta_attempts group by user_id
),
...
select jsonb_build_object(
  ...
  'content', jsonb_build_object(
    'cortices',   sc__metric(array(select (created_at at time zone 'Asia/Seoul')::date from scripta_cortices)),
    'texts',      sc__metric(array(select (created_at at time zone 'Asia/Seoul')::date from scripta_texts)),
    'paragraphs', sc__metric(array(select (created_at at time zone 'Asia/Seoul')::date from scripta_units where unit_type = 'paragraph')),
    'sentences',  sc__metric(array(select (created_at at time zone 'Asia/Seoul')::date from scripta_units where unit_type = 'sentence')),
    'chunks',     sc__metric(array(select (created_at at time zone 'Asia/Seoul')::date from scripta_units where unit_type = 'chunk'))
  ),
  ...
  'credits', jsonb_build_object(
    ...
    'spent',     (select coalesce(-sum(delta), 0)::bigint from scripta_credit_ledger where delta < 0),
    'refunded',  (select coalesce(sum(delta), 0)::bigint from scripta_credit_ledger where delta > 0 and reason like '%refund%'),
    'granted',   (select coalesce(sum(delta), 0)::bigint from scripta_credit_ledger where delta > 0 and source = 'admin_adjustment'),
    ...
    'byReason', coalesce((
      ...
      from (select reason, count(*)::int n, (-sum(delta))::bigint c
            from scripta_credit_ledger where delta < 0 group by reason) r), '[]'::jsonb),
    'dailySpent', coalesce((
      ...
      from (select (created_at at time zone 'Asia/Seoul')::date d, (-sum(delta))::bigint c
            from scripta_credit_ledger where delta < 0 group by 1) x), '[]'::jsonb)
  ),
  'payments', (
    select jsonb_build_object(
      'events', count(*)::int,
  ...
  'languages', coalesce((
    ...
    from (select target_language, count(*)::int n from scripta_cortices group by 1) l), '[]'::jsonb),
  'activation', coalesce((
