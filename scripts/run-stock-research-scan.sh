#!/bin/bash
# 종목 리서치 스캔 (launchd용 래퍼)
# 매일 09:00 실행 — portfolio_scan + 종목 발굴 리포트 → 텔레그램 전송

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
LOG_DIR="$SCRIPT_DIR/logs"
LOG_FILE="$LOG_DIR/stock-research-scan.log"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] 종목 리서치 스캔 시작" >> "$LOG_FILE"
npx tsx scripts/stock-research-scan.ts >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
