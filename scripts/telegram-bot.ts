...
const MESSAGE_BATCH_DELAY = 3000 // 메시지 배칭 디바운스 대기 시간 (ms)
...
const messageBatchBuffer: Map<number, { messages: string[]; timer: ReturnType<typeof setTimeout> }> = new Map()
...
const MORNING_BRIEF_PROMPT = `당신은 윌리입니다.
아침 브리핑을 작성하세요.
...
const FOLLOW_UP_PROMPT = `당신은 윌리입니다.
...
async function newsDigestCheck() {
  ...
  // Claude에게 요약 요청
  const prompt = `당신은 윌리입니다. 아래 뉴스/유튜브 검색 결과를 CEO에게 간결하게 브리핑하세요.
...
async function handleMessage(chatId: number, text: string) {
  ...
  try {
    ...
    const historyText = history.length
      ? '\n## 이전 대화\n' + history.map(m =>
          m.role === 'user' ? `사용자: ${m.content}` : `윌리: ${m.content}`
        ).join('\n')
      : ''
      ...
      if (existing) {
        ...
        existing.timer = setTimeout(async () => {
          ...
          if (batch) {
            ...
            await handleMessage(chatId, combined)
          }
        }, MESSAGE_BATCH_DELAY)
      } else {
