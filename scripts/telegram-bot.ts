...
// 위키 메모 후속조치 — CEO가 업무위키 'memo' 섹션에 남긴 미검토 메모를 윌리가 점검해
// 안전한 건 바로 처리(액션), 중요한 건 제안만. 처리 후 agent_reviewed_at 표시.
...
async function reviewMemos() {
  ...
  const { data: memos } = await supabase
    ...
    .eq('section', 'memo')
    .is('agent_reviewed_at', null)
    .order('created_at', { ascending: true })
    .limit(10)
  if (!memos?.length) return
  lastMemoReviewAt = Date.now()
  console.log(`📝 위키 메모 후속조치: 미검토 ${memos.length}건`)

  const memoText = memos.map((m, i) =>
    `${i + 1}. ${m.title ? `[${m.title}] ` : ''}${(m.content || '').slice(0, 800)}`).join('\n\n')
...
# 작업: 위키 메모 후속조치
아래는 CEO가 업무위키 '메모' 섹션에 남긴, 아직 처리되지 않은 메모입니다. 각 메모를 검토해서:
...
결과를 CEO에게 보낼 **간결한** 메시지로 작성하세요(메모별 1~2줄).

## 미처리 메모 (${memos.length}건)
${memoText}`
  ...
  await supabase.from('work_wiki')
    .update({ agent_reviewed_at: new Date().toISOString() })
    .in('id', memos.map(m => m.id))
  console.log(`✅ 위키 메모 ${memos.length}건 검토완료 표시`)
}
...
\`\`\`
CEO가 "메모", "메모해둬", "위키에 적어둬", "기록해둬" 라고 하면 section을 "memo"로 저장하세요. 이 메모는 나중에 윌리가 점검해 후속조치합니다.
\`\`\`action
{"type":"create_wiki","title":"짧은 제목","content":"메모 내용","section":"memo"}
\`\`\`
