...
async function checkFollowUps() {
  ...
  try {
    ...
    history.push({ role: 'assistant', content: `[팔로업]\n${response}`, timestamp: now.toISOString() })
...
async function proactiveCheck() {
  ...
  try {
    await breakingNewsCheck()
  } catch (err) {
    console.error('Breaking news check error:', err)
  }
}
...
async function marketMonitorCheck() {
  ...
  try {
    ...
    if (!briefingKey && !isWeekend && kstHour === 9 && kstMinute >= 10 && kstMinute <= 20
      ...
      // 대화 기록 저장
      const history = await getConversation(ceoChatId)
      history.push({ role: 'assistant', content: '[한국장 휴장 알림]\n오늘은 한국장 휴장이에요 🇰🇷', timestamp: new Date().toISOString() })
      await saveConversation(ceoChatId, history)
      return
    }
    ...
    if (!briefingKey && !isUsWeekend && ((kstHour === 0 && kstMinute >= 0) || (kstHour === 1 && kstMinute <= 10))
      ...
      // 대화 기록 저장
      const history = await getConversation(ceoChatId)
      history.push({ role: 'assistant', content: '[미국장 휴장 알림]\n오늘은 미국장 휴장이에요 🇺🇸', timestamp: new Date().toISOString() })
      await saveConversation(ceoChatId, history)
      return
    }
    ...
    setTimeout(async () => {
      try {
        ...
        // 대화 기록 저장
        const history = await getConversation(capturedChatId!)
        history.push({ role: 'assistant', content: `[${capturedBriefingType}]\n${response}`, timestamp: new Date().toISOString() })
        await saveConversation(capturedChatId!, history)
...
async function breakingNewsCheck() {
  ...
  const history = await getConversation(ceoChatId)
  history.push({ role: 'assistant', content: `[속보 알림]\n${alert}`, timestamp: new Date().toISOString() })
  await saveConversation(ceoChatId, history)
}
...
7. **코딩 & 시스템 관리**: 코드 수정, 파일 관리, git 작업, DB 마이그레이션 등 개발 작업 수행 가능 (풀 세션 모드)
...
async function handleMessage(chatId: number, text: string, abortSignal?: AbortSignal, lastMessageId?: number) {
  ...
  try {
    ...
    // 대시보드 + 위키 + 추적주제 + 텐소프트웍스 + 온톨로지 + 프롬프트섹션 + 속성카탈로그 + 팔로업 수집
    const [dashboardContext, wikiContext, watchTopics, tenswContext, knowledgeContext, promptSections, attrCatalog, followUpsContext] = await Promise.all([
...
# 현재 시각
${new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', weekday: 'short' })}
