...
- 표는 넓은 1개 대신 축마다 좁은 2개로 분리
- 가격표 컬럼: 종목 | 현재가 | 등락 | 고점
- 추매표 컬럼: 종목 | 수익 | T/상태 | 액션
- 종목명은 티커 또는 짧은 한글명만 사용. 회사 전체 이름 금지
- 각 종목: 현재가 + 전일대비 등락률(%) + 평균매수단가 대비 총수익률 필수
- BUY면 액션 칸에 "🔔 T{다음트랜치}" 형식으로 표시
- BUY가 아니면 액션 칸에 "없음" 또는 "대기(+N%)"처럼 짧게 명시
- 신고가/급등/급락은 이모지로 하이라이트
- 브리핑 하단에 "피라미딩 현황" 섹션을 따로 두고 BUY/HOLD/FREEZE/FULL/HOUSE_MONEY 건수를 한 줄로 요약
- 마지막에 "주요 포인트" 2~3줄 요약
- 표는 4컬럼 초과 금지. 한 메시지에 표 최대 2개만 두고, 축이 많으면 "---SPLIT---"으로 분할
- 사실 기반만. 추측 금지.
