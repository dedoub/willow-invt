...
- 법인인감은 따로 줄 필요가 없다. 공식문서함 `TS-DOC-2026-003` 에서 스크립트가 직접 받아 온다.
  못 받으면 멈춘다 — 도장 없는 명세서가 조용히 만들어지면 그대로 나간다.
...
법인인감은 공식문서함 `TS-DOC-2026-003` 이 정본이고, 스크립트가 알아서 받아 온다.
못 받으면 멈추므로 도장 빠진 명세서가 나올 일은 없다. 파일이 따로 있으면 `--seal` 로 주고,
도장 없이 초안만 뽑아 볼 때만 `--no-seal` 을 붙인다.

2026-09-22 에 `--seal` 을 빼고 돌려 도장 없는 명세서 아홉 장이 만들어졌다. 위키에 올라가고
메일 초안까지 만들어진 뒤 첨부가 164KB 에서 46KB 로 줄어든 걸 보고서야 알았다. 그래서 인자를
없애고 스크립트가 직접 받아 오게 했다.
