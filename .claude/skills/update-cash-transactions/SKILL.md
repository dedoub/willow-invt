...
description: 은행계좌내역 파일을 받아 각사별 현금관리 테이블(tensw_invoices/willow_mgmt_invoices)에 업데이트. "계좌내역 정리", "현금관리 업데이트", "은행 거래내역", "계좌 업데이트" 시 사용.
---
...
| 회사 | 테이블 | 프론트엔드 |
|------|--------|-----------|
| 텐소프트웍스 | `tensw_invoices` | /tensoftworks/management → 현금관리 |
| 윌로우 | `willow_mgmt_invoices` | /willow-investment/management → 현금관리 |
...
INSERT INTO tensw_invoices (type, counterparty, description, amount, payment_date, status, account_number, notes)
...
UPDATE tensw_invoices SET status = 'completed', payment_date = '입금일'
