...
## MCP Server
Willow Dashboard는 자체 MCP 서버를 내장하고 있으며, Codex Desktop 등 MCP 클라이언트에서 OAuth 2.1 인증 후 데이터에 접근 가능.
...
# 2단계 — 확정 급여대장이 오면 대량이체 xls + 개인별 급여명세서
...
# 3단계 — 급여일 아침, 각자에게 명세서 메일
node scripts/tensw-payslip-send.mjs --month 2026-08                           # 초안
...
- 이체 금액은 대장의 **차인지급액**. 이체 파일 여덟 칸은 모두 글자로 적는다.
