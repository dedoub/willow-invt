...
/**
 * 사업관리 NEW — 디자인을 한 페이지씩 잡아 보는 사본이다.
 * 카드 구성과 데이터는 /mgmt 와 같은 블록을 그대로 쓴다(CEO 2026-09-10: 카드 구조는 유지).
 * 여기서 바꾸는 것은 배치·여백·색·메타 위치 같은 시각뿐이고, 확정되면 /mgmt 로 옮긴다.
 */
...
import { CashBlock } from '@/app/(dashboard)/(linear)/mgmt/_components/cash-block'
import { TaxManagementBlock } from '@/app/(dashboard)/(linear)/mgmt/_components/tax-management-block'
import { SalesBlock } from '@/app/(dashboard)/(linear)/mgmt/_components/sales-block'
...
export default function MgmtNewPage() {
  ...
  const handleConfirmParsed = async (txs: ParsedTransaction[]) => {
    ...
    if (parsedBalance && parsedBankName) {
      await fetch('/api/willow-mgmt/bank-balances', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
