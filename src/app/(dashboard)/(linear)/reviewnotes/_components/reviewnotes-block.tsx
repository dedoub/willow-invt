...
// AI 열은 아래 '누적 n,nnn' 줄이 44px 를 넘겨(48px) 옆 용량 열 위에 그려지고 있었다 — 52px 로 올린다.
const USER_TABLE_COL_SPEC = '72px 72px minmax(72px,1fr) minmax(84px,1.1fr) 52px 40px 44px 40px 40px 52px 48px 48px 52px 58px'
      ...
      {!loading && trafficStats && (
        <div>
          ...
          {(() => {
            ...
            // MAU / DAU/MAU / ARPMAU — 구독을 접어 MRR이 없으니 사용자당 매출은 30일 창으로 잰다.
            ...
            // DAU/MAU 추이(점선) — 날짜마다 "그날까지 30일 평균 DAU ÷ 그날의 MAU". 배지와 같은
            // 셈을 날짜마다 되풀이한 것이라 두 값이 어긋나지 않는다. 하루치 DAU 를 그대로 나누면
            // 요일 진폭이 그대로 튀어 추세가 안 보인다. 날짜 축이 조용한 날까지 채워져 있어
          ...
          <StatRows cols={mobile ? 'repeat(2, minmax(0,1fr))' : 'repeat(3, minmax(0,1fr))'}>
            ...
            <LStat
              label="MAU"
              title={`직전 30일 활동 회원 수 (EventLog 마지막 활동 기준, 관리자 제외). DAU/MAU ${stickiness}% — 한 달에 한 번이라도 온 사람 중 하루에 오는 비율로, 분자는 오늘을 뺀 최근 ${dauRows.length}일 평균 활동자 ${avgDau.toLocaleString()}명(옆 '일별 활동자' 바와 같은 모집단)이다. 높을수록 매일 쓰는 제품에 가깝다. ARPMAU = 최근 30일 매출 ${formatCurrency(revenue30)} ÷ MAU ${mau.toLocaleString()}명. 실선은 MAU 추이, 점선은 DAU/MAU 추이(그날까지 30일 평균 DAU ÷ 그날의 MAU) — 이 행의 다른 카드는 누적이지만 두 지표 모두 누적이 뜻이 없어 롤링 30일로 그린다.`}
