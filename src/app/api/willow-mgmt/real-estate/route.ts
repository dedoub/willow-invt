...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (type === 'listing-trend') {
      ...
      // Gap calculation — same method as summary: per complex+band gap → average
      const areaMapping = await buildAreaMapping(supabase, complexNames)
      ...
      // Latest snapshot listings grouped by complex+band
      const { data: latestSnap } = await supabase
        .from('re_naver_listings').select('snapshot_date')
        .in('complex_name', complexNames)
        .order('snapshot_date', { ascending: false }).limit(1)
      const snapDate = latestSnap?.[0]?.snapshot_date
      const snapListings = snapDate ? await fetchAll(
        supabase.from('re_naver_listings')
          .select('complex_name, trade_type, price, area_supply_sqm, area_type')
          .eq('snapshot_date', snapDate).in('complex_name', complexNames)
      ) : []
      ...
      const listBands: Record<string, number[]> = {}
      for (const l of snapListings) {
        const py = getListingPyeong(l)
        if (py <= 0 || !matchesAreaT(py)) continue
        if ((tradeType === '매매' && l.trade_type !== '매매') || (tradeType === '전세' && l.trade_type !== '전세')) continue
        const ppp = Number(l.price) / py
        if (ppp <= 0) continue
        const key = `${l.complex_name}|${getBandT(py)}`
        if (!listBands[key]) listBands[key] = []
        listBands[key].push(ppp)
      }

      const actualBands: Record<string, number[]> = {}
      if (tradeType === '매매') {
        ...
        for (const t of actuals) {
          ...
          if (!actualBands[key]) actualBands[key] = []
          actualBands[key].push(Number(t.deal_amount) / supplyPy)
        }
      } else {
        const actuals = await fetchAll(applyFilters(
        ...
        for (const r of actuals) {
          ...
          if (!actualBands[key]) actualBands[key] = []
          actualBands[key].push(Number(r.deposit) / supplyPy)
        }
      }

      const gaps: number[] = []
      for (const [key, listings] of Object.entries(listBands)) {
        if (listings.length > 0 && actualBands[key]?.length > 0) {
          const minListing = Math.min(...listings)
          const avgActual = actualBands[key].reduce((s, v) => s + v, 0) / actualBands[key].length
          if (avgActual > 0) gaps.push(((minListing - avgActual) / avgActual) * 100)
        }
      }
      const gapRate = gaps.length > 0 ? Math.round(gaps.reduce((s, v) => s + v, 0) / gaps.length * 10) / 10 : null

      return NextResponse.json({ dates, complexes, tradeType, gapRate })
    }
