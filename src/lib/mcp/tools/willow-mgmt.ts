...
export function registerWillowMgmtTools(server: McpServer) {
  ...
  server.registerTool('willow_list_clients', {
    description: '[윌로우인베스트먼트 > 사업관리] 클라이언트 목록을 조회합니다',
    ...
    const perm = checkToolPermission('willow_list_clients', user, authInfo?.scopes || [])
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_clients' })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_create_client', {
    description: '[윌로우인베스트먼트 > 사업관리] 클라이언트를 생성합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_create_client', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_create_client', inputParams: input })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_update_client', {
    description: '[윌로우인베스트먼트 > 사업관리] 클라이언트를 수정합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_update_client', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_update_client', inputParams: { id, ...updates } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_delete_client', {
    description: '[윌로우인베스트먼트 > 사업관리] 클라이언트를 삭제합니다',
    inputSchema: z.object({
      id: z.string().describe('클라이언트 ID'),
    }),
  }, async ({ id }, { authInfo }) => {
    ...
    const perm = checkToolPermission('willow_delete_client', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_delete_client', inputParams: { id } })
  ...
  server.registerTool('willow_list_projects', {
    description: '[윌로우인베스트먼트 > 사업관리] 프로젝트 목록을 조회합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_list_projects', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }

    const supabase = getServiceSupabase()
    let query = supabase
      .from('willow_mgmt_projects')
      .select('*, client:willow_mgmt_clients(*), milestones:willow_mgmt_milestones(count)')
      .order('order_index')
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_projects', inputParams: { client_id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_create_project', {
    description: '[윌로우인베스트먼트 > 사업관리] 프로젝트를 생성합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_create_project', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    const { data, error } = await supabase
      .from('willow_mgmt_projects')
      .insert(input)
      .select('*, client:willow_mgmt_clients(*)')
      .single()

    if (error) return { content: [{ type: 'text' as const, text: `Error: ${error.message}` }], isError: true }

    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_create_project', inputParams: input })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_update_project', {
    description: '[윌로우인베스트먼트 > 사업관리] 프로젝트를 수정합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_update_project', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_update_project', inputParams: { id, ...updates } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_delete_project', {
    description: '[윌로우인베스트먼트 > 사업관리] 프로젝트를 삭제합니다',
    inputSchema: z.object({
      id: z.string().describe('프로젝트 ID'),
    }),
  }, async ({ id }, { authInfo }) => {
    ...
    const perm = checkToolPermission('willow_delete_project', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_delete_project', inputParams: { id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify({ success: true, deleted_id: id }) }] }
  })
  ...
  server.registerTool('willow_list_milestones', {
    description: '[윌로우인베스트먼트 > 사업관리] 마일스톤 목록을 조회합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_list_milestones', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_milestones', inputParams: { project_id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_create_milestone', {
    description: '[윌로우인베스트먼트 > 사업관리] 마일스톤을 생성합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_create_milestone', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_create_milestone', inputParams: input })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_update_milestone', {
    description: '[윌로우인베스트먼트 > 사업관리] 마일스톤을 수정합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_update_milestone', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_update_milestone', inputParams: { id, ...updates } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_delete_milestone', {
    description: '[윌로우인베스트먼트 > 사업관리] 마일스톤을 삭제합니다',
    inputSchema: z.object({
      id: z.string().describe('마일스톤 ID'),
    }),
  }, async ({ id }, { authInfo }) => {
    ...
    const perm = checkToolPermission('willow_delete_milestone', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_delete_milestone', inputParams: { id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify({ success: true, deleted_id: id }) }] }
  })
  ...
  server.registerTool('willow_list_schedules', {
    description: '[윌로우인베스트먼트 > 사업관리] 스케줄 목록을 조회합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_list_schedules', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_schedules', inputParams: { start_date, end_date, client_id } })
  ...
  server.registerTool('willow_create_schedule', {
    description: '[윌로우인베스트먼트 > 사업관리] 스케줄을 생성합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_create_schedule', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_create_schedule', inputParams: input })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_update_schedule', {
    description: '[윌로우인베스트먼트 > 사업관리] 스케줄을 수정합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_update_schedule', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_update_schedule', inputParams: { id, ...updates } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_delete_schedule', {
    description: '[윌로우인베스트먼트 > 사업관리] 스케줄을 삭제합니다',
    inputSchema: z.object({
      id: z.string().describe('스케줄 ID'),
    }),
  }, async ({ id }, { authInfo }) => {
    ...
    const perm = checkToolPermission('willow_delete_schedule', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_delete_schedule', inputParams: { id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify({ success: true, deleted_id: id }) }] }
  })

  server.registerTool('willow_toggle_schedule_date', {
    description: '[윌로우인베스트먼트 > 사업관리] 멀티데이 스케줄의 특정 날짜 완료 상태를 토글합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_toggle_schedule_date', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_toggle_schedule_date', inputParams: { schedule_id, date } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })
  ...
  server.registerTool('willow_list_tasks', {
    description: '[윌로우인베스트먼트 > 사업관리] 태스크 목록을 조회합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_list_tasks', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_tasks', inputParams: { schedule_id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_create_task', {
    description: '[윌로우인베스트먼트 > 사업관리] 태스크를 생성합니다',
    inputSchema: z.object({
      schedule_id: z.string().describe('스케줄 ID'),
    ...
    const perm = checkToolPermission('willow_create_task', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_create_task', inputParams: input })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_update_task', {
    description: '[윌로우인베스트먼트 > 사업관리] 태스크를 수정합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_update_task', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_update_task', inputParams: { id, ...updates } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_delete_task', {
    description: '[윌로우인베스트먼트 > 사업관리] 태스크를 삭제합니다',
    inputSchema: z.object({
      id: z.string().describe('태스크 ID'),
    }),
  }, async ({ id }, { authInfo }) => {
    ...
    const perm = checkToolPermission('willow_delete_task', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_delete_task', inputParams: { id } })
    return { content: [{ type: 'text' as const, text: JSON.stringify({ success: true, deleted_id: id }) }] }
  })
  ...
  server.registerTool('willow_list_memos', {
    description: '[윌로우인베스트먼트 > 사업관리] 일일 메모 목록을 조회합니다',
    inputSchema: z.object({
      start_date: z.string().optional().describe('시작일 필터 (YYYY-MM-DD)'),
      end_date: z.string().optional().describe('종료일 필터 (YYYY-MM-DD)'),
    }),
    ...
    const perm = checkToolPermission('willow_list_memos', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_memos', inputParams: { start_date, end_date } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_upsert_memo', {
    description: '[윌로우인베스트먼트 > 사업관리] 일일 메모를 생성하거나 수정합니다 (날짜 기준 upsert)',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_upsert_memo', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_upsert_memo', inputParams: input })
    return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] }
  })

  server.registerTool('willow_delete_memo', {
    description: '[윌로우인베스트먼트 > 사업관리] 일일 메모를 삭제합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_delete_memo', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_delete_memo', inputParams: { date } })
  ...
  server.registerTool('willow_list_invoices', {
    description: '[윌로우인베스트먼트 > 사업관리] 인보이스/현금관리 목록을 조회합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_list_invoices', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_list_invoices', inputParams: { type, status } })
    return { content: [{ type: 'text' as const, text: JSON.stringify(invoices, null, 2) }] }
  })

  server.registerTool('willow_create_invoice', {
    description: '[윌로우인베스트먼트 > 사업관리] 인보이스/현금관리 항목을 생성합니다',
    inputSchema: z.object({
      type: z.enum(['revenue', 'expense', 'asset', 'liability', 'transfer']).describe('유형 (revenue=수입, expense=지출, asset=자산, liability=부���, transfer=대체 - 자산종류변경/채무상환 등 순자산 불변 거래)'),
    ...
    const perm = checkToolPermission('willow_create_invoice', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_create_invoice', inputParams: input })
  ...
  server.registerTool('willow_update_invoice', {
    description: '[윌로우인베스트먼트 > 사업관리] 인보이스/현금관리 항목을 수정합니다',
    inputSchema: z.object({
    ...
    const perm = checkToolPermission('willow_update_invoice', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'willow_update_invoice', inputParams: { id, ...updates } })
    return { content: [{ type: 'text' as const, text: JSON.stringify({ ...data, amount: Number(data.amount) }, null, 2) }] }
  })

  server.registerTool('willow_delete_invoice', {
    description: '[윌로우인베스트먼트 > 사업관리] 인보이스/현금관리 항목을 삭제합니다',
    inputSchema: z.object({
      id: z.string().describe('인보이스 ID'),
    }),
  }, async ({ id }, { authInfo }) => {
    ...
    const perm = checkToolPermission('willow_delete_invoice', user, authInfo?.scopes || [])
    if (!perm.allowed) return { content: [{ type: 'text' as const, text: perm.reason! }], isError: true }
