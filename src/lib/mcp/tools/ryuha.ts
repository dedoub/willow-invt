...
export function registerRyuhaTools(server: McpServer) {
  ...
  server.registerTool('ryuha_list_schedules', {
    ...
    await logMcpAction({ userId: user.userId, action: 'tool_call', toolName: 'ryuha_list_schedules', inputParams: { start_date, end_date } })
  ...
  server.registerTool('ryuha_create_schedule', {
    ...
    inputSchema: z.object({
      ...
      type: z.enum(['school', 'academy', 'arts', 'homework', 'etc']).optional().describe('유형: school(학교), academy(학원), arts(예체능), homework(과제), etc(기타)'),
