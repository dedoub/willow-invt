...
export const agentTools = [
  ...
  // -- Sales --
  { name: 'tensw_list_sales', description: '[텐소프트웍스] 매출관리(세금계산서) 목록', parameters: { type: 'object' as const, properties: { status: { type: 'string', description: 'scheduled(발행예정)|planned(계약예정)|pending|paid' } } } },
  { name: 'tensw_create_sales', description: '[텐소프트웍스] 매출 항목 생성', parameters: { type: 'object' as const, properties: { issue_date: { type: 'string', description: '계산서 발행일 (필수)' }, counterparty: { type: 'string', description: '거래처 (필수)' }, supply_amount: { type: 'string', description: '공급가액 (필수)' }, tax_amount: { type: 'string', description: '세액' }, total_amount: { type: 'string', description: '합계' }, payment_status: { type: 'string', description: 'scheduled(발행예정)|planned(계약예정)|pending|paid' }, notes: { type: 'string', description: '비고' } }, required: ['issue_date', 'counterparty', 'supply_amount'] } },
  { name: 'tensw_update_sales', description: '[텐소프트웍스] 매출 항목 수정', parameters: { type: 'object' as const, properties: { id: { type: 'string', description: 'ID (필수)' }, issue_date: { type: 'string', description: '발행일' }, counterparty: { type: 'string', description: '거래처' }, supply_amount: { type: 'string', description: '공급가액' }, tax_amount: { type: 'string', description: '세액' }, total_amount: { type: 'string', description: '합계' }, payment_status: { type: 'string', description: 'scheduled(발행예정)|planned(계약예정)|pending|paid' }, notes: { type: 'string', description: '비고' } }, required: ['id'] } },
