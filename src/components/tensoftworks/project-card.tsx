...
const getActivityConfig = (type: string, activityT: ActivityTypeTranslations) => {
  switch (type) {
    ...
    case 'discarded':
      return { icon: <Ban className="h-4 w-4" />, color: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400', label: activityT.discarded }
    case 'analysis':
      return { icon: <Brain className="h-4 w-4" />, color: 'bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-400', label: activityT.analysis }
    case 'doc_created':
      return { icon: <FileText className="h-4 w-4" />, color: 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400', label: activityT.docCreated }
    case 'schedule_created':
      return { icon: <Calendar className="h-4 w-4" />, color: 'bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-400', label: activityT.scheduleCreated }
    case 'schedule_updated':
      return { icon: <Calendar className="h-4 w-4" />, color: 'bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-400', label: activityT.scheduleUpdated }
    case 'schedule_completed':
      return { icon: <Calendar className="h-4 w-4" />, color: 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400', label: activityT.scheduleCompleted }
    case 'commit':
      return { icon: <GitCommit className="h-4 w-4" />, color: 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300', label: activityT.commit }
    default:
      return { icon: <Bell className="h-4 w-4" />, color: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400', label: type }
  }
}
        ...
        <CardContent className="pt-0 space-y-2">
          ...
          {project.project_url && !project.serviceUrls?.length && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-sm text-amber-700 dark:text-amber-300 hover:text-amber-900 dark:hover:text-amber-200"
            >
          ...
          {project.memo && (
            <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-2">{project.memo}</p>
          )}
          {/* Members */}
      ...
      <CardContent className="pt-0 space-y-3 overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            {project.inProgressByMember && project.inProgressByMember.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {project.inProgressByMember.slice(0, 2).map((m, i) => (
                  <span key={i} className="text-sm bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-400 px-1.5 py-0.5 rounded">
                    {m.name} {m.count}
                  </span>
                ))}
                {project.inProgressByMember.length > 2 && (
                  <span className="text-sm text-blue-500 dark:text-blue-400">+{project.inProgressByMember.length - 2}</span>
                )}
              </div>
            )}
          </div>
            ...
            {project.completedByMember && project.completedByMember.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {project.completedByMember.slice(0, 2).map((m, i) => (
                  <span key={i} className="text-sm bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400 px-1.5 py-0.5 rounded">
                    {m.name} {m.count}
                  </span>
                ))}
                {project.completedByMember.length > 2 && (
                  <span className="text-sm text-emerald-500 dark:text-emerald-400">+{project.completedByMember.length - 2}</span>
                )}
              </div>
            )}
          </div>
          ...
          <div className="p-3 rounded-lg bg-slate-200 dark:bg-slate-700">
            ...
            <div className="text-2xl font-bold text-slate-700 dark:text-slate-300">{progress}%</div>
                ...
                <div className="space-y-1">
                  {project.schedules.slice(0, 3).map(schedule => (
                    <div key={schedule.id} className="flex items-center gap-2 text-sm">
                      <span className="text-slate-400 dark:text-slate-500 w-12 flex-shrink-0">
              ...
              {hasDocs ? (
                <div className="flex flex-wrap gap-1">
                  {project.docs.slice(0, 4).map(doc => (
                    <span
                      key={doc.id}
                      className="text-sm px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300"
                    >
                      {doc.title}
                    </span>
                  ))}
                  {project.docs.length > 4 && (
                    <span className="text-sm px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-400 dark:text-slate-500">
                      +{project.docs.length - 4}
                    </span>
                  )}
                </div>
                  ...
                  return (
                    <div key={activity.id} className={`p-2 rounded ${config.color}`}>
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {activity.changed_by && (
                          <span className="text-sm opacity-70">· {activity.changed_by}</span>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${priorityDot}`} />
                        <p className="text-sm truncate font-medium opacity-90" title={activity.title}>
                          {activity.title}
                        </p>
                      </div>
              ...
              {project.todos.map(todo => {
                ...
                return (
                  <div key={todo.id} className="flex items-center gap-2 text-sm">
                    {todo.readable_id && (
                      <span className="text-blue-600 dark:text-blue-400 font-mono text-sm flex-shrink-0">
                        [{todo.readable_id}]
                      </span>
                    )}
                    <span className="truncate text-slate-700 dark:text-slate-200 min-w-0 flex-1">{todo.title}</span>
                    <span className={`text-sm px-2 py-0.5 rounded flex-shrink-0 ${priorityBadge}`}>
                      {priorityLabel}
                    </span>
                    {todo.due_date && (
                    ...
                    {todo.assignees && todo.assignees.length > 0 && (
                      <span className="text-sm text-slate-500 dark:text-slate-400 flex-shrink-0 max-w-[80px] truncate">
                        {todo.assignees.join(', ')}
                      </span>
                    )}
                  </div>
        ...
        {/* Memo */}
        {project.memo && (
          <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2">{project.memo}</p>
        )}
      </CardContent>
    </Card>
  )
