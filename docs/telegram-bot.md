...
| `scripts/logs/telegram-bot.runtime.jsonl` | 구조화 런타임 로그 (경고/오류/처리이력) |
...
**메시지 배칭**: 1초 debounce로 빠른 연속 메시지를 합쳐서 한 번에 Claude에 전달한다.

**실시간 처리현황**: 수신 직후 "받음 → 컨텍스트 수집 → Claude 처리 → 액션 실행 → 응답 전송" 단계를 텔레그램 진행 메시지로 계속 갱신한다.

**구조화 런타임 로그**: `console` 출력과 주요 처리 이벤트를 `telegram-bot.runtime.jsonl`에 함께 남긴다. 최근 경고/오류 요약은 프롬프트의 "봇 런타임 로그" 섹션으로 주입되어, 장애 진단 시 봇이 자기 로그를 먼저 참고할 수 있다.
...
| 앱 버그 감시 | 15-20분 간격 | VoiceCards / ReviewNotes 사용자 로그 기반 시스템 버그 알림 |

앱 버그 감시 규칙:
- VoiceCards: 최근 실사용 이벤트 급감, Google Drive 권한 누락, 가입 대비 업로드 부진, 로컬 앱 로그 오류 감시(옵션)
- ReviewNotes: `WebhookEvent`의 `system_bug` payload 기반 클라이언트/서버 버그, 반복 API 실패, 웹훅 처리 정체 감시
...
| `MESSAGE_BATCH_DELAY` | 1,000ms | 메시지 배칭 딜레이 |
...
- `tensw-weekly-reports`
  대상 스크립트가 없어 레지스트리에서 숨겨 두며, 구현 복구 후 `service_enable`로 다시 열 수 있다.

서비스 레지스트리는 `is_enabled=false` 엔트리를 실제 제어 목록에서 제외한다.
