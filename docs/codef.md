...
| 홈택스 매출·매입 | 2 |
| 카드 승인내역 | 1 |
| 카드 명세서 (최근 2개월 × 결제그룹) | ~6 |
| 합계 | **~18** |

명세서는 확정되면 안 바뀌므로 기본 2개월만 본다(`--billing-months` 로 조정).
카드도 결제그룹 수만큼만 돌고 새 명세서가 두 장 연속 안 나오면 멈춘다.
초기 설계는 8장 × 4개월 = 32콜이었고, 2026-08-20 아침에 실제로 한도를 터뜨렸다.
...
2026-08-20에 한도 초과로 신한 계좌·세금계산서·카드가 통째로 안 돌았는데 아무도 몰랐다.
