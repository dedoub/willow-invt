...
**날짜**: 2026-09-03
**상태**: 승인됨 (2026-09-03). 1단계 기반 구현 완료(스키마·CLI·시드). 3단계 중 열람 페이지 `/corp`와 읽기 API를 먼저 배포(2026-09-03). 2단계 의사결정 파이프라인 계획 대기
...
[두뇌] 세션 에이전트가 스킬 절차대로 CLI 스크립트를 호출
       ...
       4. corp action add (sign/confirm/provide) → 세션에서 CEO에게 요청 사항 출력
                              ↓
[사용자] 초안 PDF 열어 확인(웹 /corp 또는 윌리가 보낸 파일) → "확인" 한마디(confirm 액션 완료)
         → 기본: 에이전트가 인감 이미지를 합성한 확정본을 렌더해 final_signed로 추가 (12절 4-1)
         → 예외(등기 제출용 등): 출력·실물 날인 → 스캔 PDF를 세션 또는 윌리에게 건네며 "서명본이야"
...
4-1. **인감 이미지와 이미지 날인 (CEO 결정 2026-09-03)**: 양사 법인인감(대표이사 인) 이미지를 상시 서류 `other`(태그 `seal`)로 보관하고, **에이전트가 확정 문서에 인감 이미지를 합성해 날인한다.** 흐름: 초안 PDF → CEO `confirm` 액션 완료 → 에이전트가 인감 이미지를 합성한 확정본 PDF를 렌더 → `final_signed` 버전(generated_by `agent`, note에 `sealed_by_image`) 추가 → 이벤트 `finalized`. 물리 서명·스캔 업로드 경로도 그대로 두며, 둘 중 어느 쪽이든 확정본은 append-only다. 인감 이미지 원본은 투명 배경 PNG로 정리해 `scripts/logs/corp-records/`(gitignore)에 두고 서류함에도 등록한다. 단서: 등기소·관공서 제출용 문서는 법무사 안내에 따라 실물 인감 날인본이 별도로 필요할 수 있으며, 그 경우 스캔본을 새 버전으로 추가한다. 인감 이미지는 CEO의 `confirm` 없이는 어떤 문서에도 합성하지 않는다.
