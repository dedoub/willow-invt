...
|---|---|
| id, company | |
| rule_type | `articles`(정관) \| `bonus_regulation`(임원상여금지급규정) \| `board_regulation` \| `compensation_regulation` \| `other` |
| title, version_no | |
...
`.claude/skills/corp-records/SKILL.md`에 두고, 규정 원문과 회사 사실은 DB에서 읽는다. 초기 규칙:

| 입력 유형 | 판단 |
|---|---|
| 임원 보수(연봉) 변경 | 정관 "임원의 보수는 주주총회 결의" → 주주총회 결의 필요. 주주 3인·자본금 10억 미만이면 상법 363조④⑤에 따라 소집절차 생략 동의 + 서면결의 가능 → `waiver_notice` + `written_resolution_shareholders` + `minutes_shareholders` + `compensation_notice`(또는 `exec_contract` 갱신) |
| 임원 상여 지급 | 임원상여금지급규정의 정기·성과상여 조항 확인 → 규정이 정한 결의(주총 또는 이사회) 문서 + `bonus_payment_resolution` |
| 정관 변경(사업목적 추가 등) | 주주총회 특별결의 → 결의 문서 + 신 정관 전문 `regulation` v(n+1) + rules 새 버전(effective_from=결의일 또는 등기일) + 등기 필요 표시(`needs_registration=true`, 상시 서류에 `registry_extract` 재발급 action) |
| 이사·감사 선임/퇴임, 본점 이전 | 주주총회 결의 + 등기 필요 |
| 이사회 | 이사 3인 미만이면 이사회 없음(상법 383조). 이사회 필요 안건은 주주총회 또는 이사 결정서로 대체하고 그 근거를 basis에 기록 |
| 계약 체결 | 결의 불필요가 기본. 임원과의 계약(자기거래)은 이사회 승인(이사회 없으면 주주총회) 필요 → 규정 확인 후 판단 |
| 세무 | 신고서·납부확인서 보관. 결의 불필요 |

에이전트는 판단 근거(조항 인용 + 회사 사실)를 `agent_plan.reasoning`과 `basis`에 남긴다. 규정에서 근거를 찾지 못하면 임의 판단하지 않고 `provide` action으로 사용자에게 묻는다.
...
1. **정관 전문, 임원상여금지급규정** — 이 기기에서 파일을 찾지 못했다. CEO가 PDF 또는 텍스트를 제공하면 rules v1(effective_from=제정·최근 개정일)로 등록하고 조 단위로 파싱한다.
2. 등기부등본(2026-06-04 발급, 이미지 PDF → OCR), 사업자등록증(2025-09-08, 2026-06-10 두 버전), 주주명부(2026-07-17), 대표이사 계약서(docx → PDF 변환)를 상시 서류로 등록하고 profile 스냅샷을 만든다.
...
1. 정관과 임원상여금지급규정 파일 제공 필요. 없으면 등기부 OCR과 CEO 구술로 임시 등록하고 `provide` action으로 남긴다.
