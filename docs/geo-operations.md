...
전체 퍼널은 여기서 끝나지 않는다: 추천 Top3 → URL 인용 → AI 유입 클릭 → 가입 → 활성화. AI 유입 클릭은 Umami의 ChatGPT·Gemini·Perplexity 등 AI 리퍼러 세션을 정본으로 집계하고, `vc_crawl_log` referral은 계측 폴백으로만 사용한다.
...
- **리뷰노트는 ChatGPT 30문항 전량 미등장이다.** 언급·Top3·인용 전부 0%. 지난주 3회차와 같다.
  Gemini만 Top3 0% → 1.7%, 인용 0% → 3.3%로 처음 0을 벗어났는데 질문 1개짜리라 노이즈 범위다.
  여기에 의미를 부여하면 지난주 1회차 과장과 같은 실수가 된다.
  **두 엔진을 합쳐 보면 리뷰노트는 여전히 진입 자체를 못 하고 있다** — 질문군을 다시 골라야
  한다는 기존 결론이 두 주 연속 유지됐다.
...
이번 주 대비가 선명하다. **보이스카드는 뾰족한 한 군에서 확실히 잡히고, 리뷰노트는 어느
엔진에서도 안 잡힌다.** 처방이 갈린다 — 보이스카드는 이미 이기는 alt 니치를 넓히는 문제고,
리뷰노트는 지금 질문군에서 이길 수 있는지 자체를 다시 봐야 한다.

**08-07 갱신 (여전히 측정 중).** 크론이 Gemini 2회차를 채워 사이트별 60 → 90행이 됐다.
아직 3회차가 남아 있어 확정값이 아니다.

| 사이트 | 엔진 | 행 | 언급 | 추천 Top3 | 인용 |
|---|---|---|---|---|---|
| voicecards | chatgpt | 30 | 23.3% | **20%** | 20% |
| voicecards | gemini | 90 | 8.9% | 5.6% | **0%** |
| reviewnotes | chatgpt | 30 | 0% | 0% | 0% |
| reviewnotes | gemini | 90 | 5.6% | 1.1% | 4.4% |

30행이 더 쌓였는데 결론이 하나도 안 바뀌었다. 보이스카드 Top3는 전주 10.0% → 9.2%로 회차
노이즈 범위 안에서 횡보하고, **Gemini 인용은 90번 중에도 0건**이라 "회차를 더 쌓아도 안
움직인다"는 위 판단이 세 번째로 확인됐다. 리뷰노트 Gemini는 1.7% → 1.1%로 오히려 내려갔다 —
08-05에 "질문 1개짜리라 노이즈"라고 유보한 게 맞았다.

색인 쪽 변명은 이번 주로 끝났다. 리뷰노트 로케일 전수 스캔·사이트맵 재제출로 색인이 17 →
117쪽(원본 20)이 됐는데도 ChatGPT 30문항 전량 0이다. **인용할 대상이 없어서가 아니다.**
남은 설명은 외부 신뢰도뿐이고, 그건 사이트 안에서 못 고친다.
...
- **AI 유입 → 가입 → 활성화 미연결.** 클릭 이후 구간이 비어 있다. 양쪽 앱에 first-touch UTM 캡처가
  들어가야 이어진다. 클릭 수는 Umami 리퍼러로 집계하지만, 가입·활성화까지의 first-touch 연결은 아직 미구현이다.
