...
### 2026-07-28 (기준선, 30문항 전량 · 1회차)

무료 티어로는 8문항·3문항까지밖에 못 갔다. 보이스카드 결제 키로 경로를 바꾸고 나서야
양쪽 30문항을 다 돌렸다. 이게 실질적인 기준선이다.

| 사이트 | 질문 | 언급 | 추천 Top3 | 인용 |
|---|---|---|---|---|
| voicecards | 30 | 10.0% | 6.7% | 0% |
| reviewnotes | 30 | 6.7% | 3.3% | 6.7% |

단계별로 보면 대부분이 미등장이다.

| 사이트 | 추천 Top3 | 언급만 | 인용만 | 미등장 |
|---|---|---|---|---|
| voicecards | 2 | 1 | 0 | 27 |
| reviewnotes | 1 | 1 | 0 | 28 |

숫자보다 **어디서 걸리느냐**가 갈린다.

- **보이스카드는 언급되는데 한 번도 인용되지 않는다.** 걸린 3문항 전부 `alt`군(앤키 대안)이고,
  인용은 0이다. 모델이 브랜드는 알지만 그 근거가 우리 페이지가 아니라는 뜻이다.
  같은 답변에 anki·quizlet·brainscape·speechling이 함께 나온다.
- **리뷰노트는 한국어 질문에서만 잡힌다.** 걸린 2문항 다 `ko`군이고 둘 다 인용까지 갔다.
  영어 질문 전량은 미등장이다.

두 경우 다 처방은 사이트 안이 아니라 밖에 있다 — 색인 쪽 진단(백링크가 손 안 댄 유일한 레버)과
같은 결론이다. 리뷰노트 영어권은 그 위에 언어별 콘텐츠 문제가 하나 더 얹혀 있다.
